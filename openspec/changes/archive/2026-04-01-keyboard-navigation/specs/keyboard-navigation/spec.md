...
`iced::keyboard::on_key_press` subscription that is active only while the relevant
screen or dialog is displayed.
...
that is empty. If all text inputs have pre-filled values, focus SHALL move to either
the primary CTA button (if iced supports button focus) or the first text input in the
ring. This focus SHALL be applied via a `text_input::focus(id)` Task returned from
the `update()` call that opens the dialog.
...
- **THEN** focus is placed on the first text input in the ring (or the primary CTA button
  if button focus is supported by iced)
